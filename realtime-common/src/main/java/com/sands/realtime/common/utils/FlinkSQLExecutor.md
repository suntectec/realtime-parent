...
**Flink S3 Hadoop dependency policy**: plugins/s3-fs-hadoop/flink-s3-fs-hadoop only for DataStream API, which does not support Table API & SQL.
...
Issue resolved by change flink-connector-kafka to flink-sql-connector-kafka dependency.
